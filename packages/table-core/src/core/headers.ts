...
import { CoreTable, TableFeature } from './table'

export interface CoreHeaderGroup<TData extends RowData> {
  depth: number
  headers: CoreHeader<TData, unknown>[]
  id: string
}
...
export interface CoreHeader<TData extends RowData, TValue extends CellValue> {
  /**
   ...
   * @link [Guide](https://tanstack.com/table/v8/docs/guide/headers)
   */
  subHeaders: CoreHeader<TData, TValue>[]
}
...
function createCoreHeader<TData extends RowData, TValue extends CellValue>(
  ...
  let header: CoreHeader<TData, TValue> = {
    ...
    rowSpan: 0,
    headerGroup: null!,
    getLeafHeaders: (): CoreHeader<TData, TValue>[] => {
      const leafHeaders: CoreHeader<TData, TValue>[] = []

      const recurseHeader = (h: CoreHeader<TData, any>) => {
        if (h.subHeaders && h.subHeaders.length) {
          h.subHeaders.map(recurseHeader)
        }
        leafHeaders.push(h as CoreHeader<TData, TValue>)
      }
    ...
    getContext: () => ({
      table,
      header: header as CoreHeader<TData, TValue>,
      column,
    }),
  }
  ...
  return header as CoreHeader<TData, TValue>
}

export const Headers: TableFeature<any> = {
  createTable: <TData extends RowData>(table: CoreTable<TData>): void => {
    // Header Groups
    ...
    table.getHeaderGroups = memo(
      ...
      (allColumns, leafColumns, left, right) => {
        ...
        const headerGroups = buildCoreHeaderGroups(
          allColumns,
    ...
    table.getCenterHeaderGroups = memo(
      ...
      (allColumns, leafColumns, left, right) => {
        ...
        return buildCoreHeaderGroups(allColumns, leafColumns, table, 'center')
      },
      {
...
export function buildCoreHeaderGroups<TData extends RowData>(
  allColumns: CoreColumn<TData, unknown>[],
  columnsToGroup: CoreColumn<TData, unknown>[],
  table: CoreTable<TData>,
  ...
  const findMaxDepth = (columns: CoreColumn<TData, unknown>[], depth = 1) => {
  ...
  let headerGroups: CoreHeaderGroup<TData>[] = []

  const createHeaderGroup = (
    headersToGroup: CoreHeader<TData, unknown>[],
    depth: number
  ) => {
    // The header group we are creating
    const headerGroup: CoreHeaderGroup<TData> = {
      depth,
    ...
    const pendingParentHeaders: CoreHeader<TData, unknown>[] = []
    ...
    headersToGroup.forEach(headerToGroup => {
      ...
      let column: CoreColumn<TData, unknown>
      ...
      if (
        ...
        const header = createCoreHeader(table, column, {
